  ...
  const handleCreateRenewal = async () => {
    if (!lease?.id) {
      toast({
        ...
        description: 'Assign and save the lease details before creating a renewal envelope.',
        variant: 'destructive',
      })
      return
    }
    try {
      ...
      if (!res.ok || !payload.success) {
        throw new Error(payload.error || 'Failed to create renewal.')
      }
      toast({
        title: payload.existing ? 'Renewal already exists' : 'Renewal created',
        description: `Renewal envelope is ready (status: ${payload.data?.status || 'sent_to_tenant'}).`,
      })
      router.push('/dashboard/manager/lease-renewals')
    } catch (err) {
      toast({
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            <Card>
              <CardHeader>
                <CardTitle>Lease Renewal (Digital Sign)</CardTitle>
                <CardDescription>Create a renewal envelope for tenant signing and countersign when ready.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <div className="rounded-lg border p-3 bg-slate-50">
                  <p className="font-medium">Workflow</p>
                  <p className="text-xs text-muted-foreground">
                    1) Create envelope → 2) Tenant signs → 3) Management countersigns → 4) Completed PDF.
                  </p>
                </div>
                ...
                <Button variant="outline" className="w-full" onClick={() => router.push('/dashboard/manager/lease-renewals')}>
                  View renewals
                </Button>
              </CardContent>
            </Card>
