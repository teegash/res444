...
export default function TenantLeaseRenewalPage() {
  ...
  const canSign = renewal?.status === 'sent_to_tenant'
  ...
  const sign = async () => {
    const ok = window.confirm(
      'By clicking OK, you confirm you have reviewed this renewal agreement and you intend to sign electronically.'
    )
    if (!ok) return

    try {
      setWorking(true)
      const res = await fetch(`/api/lease-renewals/${encodeURIComponent(renewalId)}/tenant-sign`, {
        method: 'POST',
        credentials: 'include',
      })
      const json = await res.json().catch(() => ({}))
      if (!res.ok || !json.ok) throw new Error(json.error || 'Signing failed.')
      toast({
        title: 'Signed',
        description: 'Your signature has been applied. Management will countersign to complete the renewal.',
      })
      await load()
    } catch (e) {
      toast({
        title: 'Signing failed',
        description: e instanceof Error ? e.message : 'Unable to sign.',
        variant: 'destructive',
      })
    } finally {
      setWorking(false)
  ...
  const timeline = useMemo(() => events.map((e) => ({ ...e, created_at: formatDate(e.created_at) })), [events])
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold">Lease Renewal</h1>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {canSign ? (
                  <Button size="sm" onClick={sign} disabled={working}>
                  ...
                  {timeline.map((e, idx) => (
                    <li key={`${e.action}-${idx}`} className="rounded-md border p-3 bg-white">
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <div className="text-xs text-muted-foreground">{e.created_at}</div>
                      </div>
                    </li>
                  ))}
                </ul>
