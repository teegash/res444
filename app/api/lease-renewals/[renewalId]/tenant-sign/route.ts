...
export async function POST(req: Request, { params }: { params: { renewalId: string } }) {
  try {
    ...
    if (r.tenant_user_id !== actorUserId) {
      return json({ error: "Forbidden: only the tenant assigned to this renewal can sign." }, 403);
    }

    const member = await isOrgMember(admin, r.organization_id, actorUserId);
    if (!member) return json({ error: "Forbidden: user is not a member of this organization." }, 403);
    ...
    if (!p12base64 || !p12pass) {
      return json({ error: "Missing tenant signing certificate env vars" }, 500);
    }
