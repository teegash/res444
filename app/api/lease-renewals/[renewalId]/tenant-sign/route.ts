...
async function downloadPdf(supabase: any, path: string) {
  const { data, error } = await supabase.storage.from("lease-renewals").download(path);
  if (error) throw new Error(`Storage download failed: ${error.message}`);
  return Buffer.from(await data.arrayBuffer());
}

async function uploadPdf(supabase: any, path: string, bytes: Uint8Array | Buffer) {
  const { error } = await supabase.storage.from("lease-renewals").upload(path, bytes, {
...
export async function POST(req: Request, { params }: { params: { renewalId: string } }) {
  try {
    ...
    if (!supabase) return json({ error: "Admin client not configured (missing env vars)" }, 500);
    ...
    if (renewal.status === "in_progress" || renewal.status === "completed" || renewal.status === "tenant_signed") {
      return json({
        ok: true,
        alreadySigned: true,
    ...
    if (renewal.status !== "sent_for_signature" && renewal.status !== "sent_to_tenant") {
    ...
    if (!renewal.pdf_unsigned_path) {
      return json(
        {
          error:
            "Missing pdf_unsigned_path on renewal. Create step must generate/upload unsigned.pdf first.",
        },
        400
      );
    }
    ...
    if (!p12base64 || !p12pass) {
      return json(
        {
          error:
            "Missing TENANT_SIGN_P12_BASE64 or TENANT_SIGN_P12_PASSWORD (or TENANT_P12_BASE64 / TENANT_CERT_PASSWORD)",
        },
        500
      );
    }

    const pdfBuffer = await downloadPdf(supabase, renewal.pdf_unsigned_path);
    ...
    await uploadPdf(supabase, tenantSignedPath, signedPdf);
